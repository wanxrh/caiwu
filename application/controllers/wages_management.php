            ...
            try{
                ...
                for ($ii = 2; $ii <= $data->sheets[0]['numRows']; $ii++){//下面有备份
                    ...
                    $user_id=$r_user['user_id'];
                        $s="";
                        if($ii==2){
                            for($j2=1;$j2<=$data->sheets[0]['numCols'];$j2++){
                                $flag='1';
                                //获取表字段
                                $sql="SHOW  full COLUMNS FROM ab22_gongzibiao";
                                $res = $this->home_model->sqlQueryArray($sql);
                                foreach ($res as $key => $row) {
                                    if($row['Comment']=$data->sheets[0]['cells'][$ii][$j2]){
                                        $flag='2';
                                    }
                                }
                                if($flag=='1'){
                                    showmsg("模版中存在错误的字段".$data->sheets[0]['cells'][$ii][$j2]."!","/wages_management/import",0,1000);exit;
                                }
                            }
                        }
                        if(!empty($user_id)){
